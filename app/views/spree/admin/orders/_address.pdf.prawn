...
bounding_box [0,600], :width => 540 do
  ...
  bounding_box [0,0], :width => 540 do
    ...
    if anonymous and Spree::Config[:suppress_anonymous_address]
      ...
      data2 << ["#{@order.bill_address.city}, #{(@order.bill_address.state)} ",
                  "#{@order.bill_address.city}, #{(@order.bill_address.state)}"]
      data2 << ["#{@order.bill_address.country.name} - #{(@order.ship_address.zipcode)} ",
                  "#{@order.bill_address.country.name} - #{(@order.ship_address.zipcode)}"]           
      data2 << ["#{@order.bill_address.phone} ",
